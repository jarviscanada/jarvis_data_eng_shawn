...
# Architecture
![application architecture](/springboot/assets/trading_app_Arch.jpg)
...
# Docker Deployment
![Docker](/springboot/assets/Trading_App_Docker_Arch.PNG)
Docker is used to manage application deployment. The above diagram shows the working of docker deployment at the backend. This application has 2 containers: one psql container and the application container. To make this application run, In order for both of these containers to communicate, a trading network is created. To assemble the images of psql, base image from dockerhub is used and other scripts that  initialise the database. Similarly, to assemble the image of application, used base image of jdk and maven and run my own scripts to package up the application.
